...
func ValidateFlagCombinations(flags *pflag.FlagSet) {
	...
	options.CheckExclusiveFlags(flags, options.REDIRECT_SCHEMA, options.EXCLUDE_SCHEMA, options.EXCLUDE_SCHEMA_FILE, options.EXCLUDE_RELATION, options.EXCLUDE_RELATION_FILE)
	if flags.Changed(options.REDIRECT_SCHEMA) &&
		...
		gplog.Fatal(errors.Errorf("Cannot use redirect-schema without exactly one include filter"), "")
	}
}
