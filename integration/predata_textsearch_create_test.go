...
var _ = Describe("backup integration create statement tests", func() {
	...
	Describe("PrintCreateTextSearchParserStatements", func() {
		parser := backup.TextSearchParser{
			Oid: 0,
			Schema: "public",
			Name: "testparser",
			StartFunc: "prsd_start",
			TokenFunc: "prsd_nexttoken",
			EndFunc: "prsd_end",
			LexTypesFunc: "prsd_lextype",
			HeadlineFunc: "prsd_headline",
		}
		...
		It("creates a basic text search parser with a comment", func() {
			parserMetadata := testutils.DefaultMetadata("TEXT SEARCH PARSER",
	...
	Describe("PrintCreateTextSearchTemplateStatement", func() {
		template := backup.TextSearchTemplate{
			Oid: 1,
			Schema: "public",
			Name: "testtemplate",
			InitFunc: "dsimple_init",
			LexizeFunc: "dsimple_lexize",
		}
		It("creates a basic text search template", func() {
			...
			testhelper.AssertQueryRuns(connectionPool, buffer.String())
			defer testhelper.AssertQueryRuns(connectionPool,
				"DROP TEXT SEARCH TEMPLATE public.testtemplate")
		...
		It("creates a basic text search template with a comment", func() {
			templateMetadata := testutils.DefaultMetadata("TEXT SEARCH TEMPLATE",
				false, false, true, false)
			...
			uniqueID := testutils.UniqueIDFromObjectName(connectionPool,
				"public", "testtemplate", backup.TYPE_TSTEMPLATE)
	...
	Describe("PrintCreateTextSearchDictionaryStatement", func() {
		dictionary := backup.TextSearchDictionary{
			Oid: 1,
			Schema: "public",
			Name: "testdictionary",
			Template: "pg_catalog.snowball",
			InitOption: "language = 'russian', stopwords = 'russian'",
		}
		...
		It("creates a basic text search dictionary with a comment and owner", func() {
			dictionaryMetadata := testutils.DefaultMetadata("TEXT SEARCH DICTIONARY",
			...
			Expect(resultDictionaries).To(HaveLen(1))
			uniqueID := testutils.UniqueIDFromObjectName(connectionPool,
				"public", "testdictionary", backup.TYPE_TSDICTIONARY)
			resultMetadata := resultMetadataMap[uniqueID]
			structmatcher.ExpectStructsToMatchExcluding(&dictionary, &resultDictionaries[0], "Oid")
	...
	Describe("PrintCreateTextSearchConfigurationStatement", func() {
		configuration := backup.TextSearchConfiguration{
			Oid: 1,
			Schema: "public",
		...
		It("creates a basic text search configuration with a comment and owner", func() {
			configurationMetadata := testutils.DefaultMetadata("TEXT SEARCH CONFIGURATION",
				false, true, true, false)
			...
			Expect(resultConfigurations).To(HaveLen(1))
			uniqueID := testutils.UniqueIDFromObjectName(connectionPool,
				"public", "testconfiguration", backup.TYPE_TSCONFIGURATION)
			resultMetadata := resultMetadataMap[uniqueID]
			structmatcher.ExpectStructsToMatchExcluding(&configuration, &resultConfigurations[0], "Oid")
