...
var _ = Describe("backup integration create statement tests", func() {
	...
	Describe("PrintRegularTableCreateStatement", func() {
		...
		It("creates a table with no attributes", func() {
			...
			testTable.Oid = testutils.OidFromObjectName(connectionPool,
				"public", "testtable", backup.TYPE_RELATION)
			resultTable := backup.ConstructDefinitionsForTables(connectionPool, []backup.Relation{testTable.Relation})[0]
			structmatcher.ExpectStructsToMatchExcluding(testTable.TableDefinition,
				resultTable.TableDefinition, "ColumnDefs.Oid", "ExtTableDef")
		})
		...
		It("creates a complex heap table", func() {
			rowOneDefault := backup.ColumnDefinition{
				Oid: 0,
				Num: 1,
				Name: "i",
				NotNull: false,
			...
			rowNotNullDefault := backup.ColumnDefinition{
				Oid: 0,
				Num: 2,
				Name: "j",
				NotNull: true,
			...
			rowNonDefaultStorageAndStats := backup.ColumnDefinition{
				Oid: 0,
				Num: 3,
				Name: "k",
				NotNull: false,
		...
		It("creates a basic append-optimized column-oriented table", func() {
			rowOne := backup.ColumnDefinition{
				...
				HasDefault: false,
				Type: "integer",
				Encoding: "compresstype=zlib,blocksize=32768,compresslevel=1",
				StatTarget: -1,
				StorageType: "",
				DefaultVal: "",
				Comment: "",}
		...
		It("creates a one-level partition table", func() {
          ...
          DEFAULT PARTITION other  WITH (tablename='public.rank_1_prt_other', appendonly=%[1]s )
			...
			testhelper.AssertQueryRuns(connectionPool, buffer.String())
			testTable.PartitionLevelInfo.Oid = testutils.OidFromObjectName(connectionPool,
				"public", "testtable", backup.TYPE_RELATION)
			testTable.Oid = testutils.OidFromObjectName(connectionPool,
				"public", "testtable", backup.TYPE_RELATION)
			resultTable := backup.ConstructDefinitionsForTables(connectionPool, []backup.Relation{testTable.Relation})[0]
...
`
		...
		It("creates a foreign table", func() {
			...
			rowOne := backup.ColumnDefinition{
				...
				DefaultVal: "",
				Comment: "",
				FdwOptions: "option1 'value1', option2 'value2'",
			}
			...
			metadata := testutils.DefaultMetadata("TABLE", true,
				true, true, true)
	...
	Describe("PrintPostCreateTableStatements", func() {
		var (
			...
			tableRow      = backup.ColumnDefinition{
				Oid: 0,
				Num: 1,
				Name: "i",
				NotNull: false,
		...
		It("prints only owner for a table with no comment or column comments", func() {
			...
			testhelper.AssertQueryRuns(connectionPool, buffer.String())
			testTableUniqueID := testutils.UniqueIDFromObjectName(connectionPool,
				"public", "testtable", backup.TYPE_RELATION)
			...
			resultTable := backup.ConstructDefinitionsForTables(connectionPool, []backup.Relation{testTable.Relation})[0]
			structmatcher.ExpectStructsToMatchExcluding(&testTable.TableDefinition,
				&resultTable.TableDefinition, "ColumnDefs.Oid", "ColumnDefs.ACL", "ExtTableDef")
		})
		It("prints table comment, table privileges, table owner, table security label, and column comments for a table", func() {
			tableMetadata = testutils.DefaultMetadata("TABLE", true,
				true, true, includeSecurityLabels)
		...
		It("prints column level security label", func() {
			testutils.SkipIfBefore6(connectionPool)
			securityLabelColumnOne := backup.ColumnDefinition{
				Oid: 0,
				Num: 1,
				Name: "i",
				Type: "integer",
	...
	Describe("PrintCreateViewStatements", func() {
		...
		It("creates a view with privileges, owner, security label, and comment", func() {
			...
			view.Oid = testutils.OidFromObjectName(connectionPool,
				"public", "simpleview", backup.TYPE_RELATION)
		...
		It("creates a view with options", func() {
			...
			view := backup.View{
				...
				Schema: "public",
				Name: "simpleview",
				Options: " WITH (security_barrier=true)",
				Definition: viewDef,
			}
	...
	Describe("PrintMaterializedCreateViewStatements", func() {
		...
		It("creates a view with privileges, owner, security label, and comment", func() {
			view := backup.MaterializedView{
				Oid: 1,
				Schema: "public",
				Name: "simplemview",
				Definition: " SELECT 1;",
			}
			viewMetadata := testutils.DefaultMetadata("MATERIALIZED VIEW",
			...
			view.Oid = testutils.OidFromObjectName(connectionPool, "public",
				"simplemview", backup.TYPE_RELATION)
			Expect(resultViews).To(HaveLen(1))
			resultMetadata := resultMetadataMap[view.GetUniqueID()]
			structmatcher.ExpectStructsToMatch(&view, &resultViews[0])
			structmatcher.ExpectStructsToMatch(&viewMetadata, &resultMetadata)
		...
		It("creates a materialized view with options", func() {
			...
			view.Oid = testutils.OidFromObjectName(connectionPool,
				"public", "simplemview", backup.TYPE_RELATION)
			Expect(resultViews).To(HaveLen(1))
			structmatcher.ExpectStructsToMatch(&view, &resultViews[0])
		})
	})
	Describe("PrintCreateSequenceStatements", func() {
		...
		It("creates a basic sequence", func() {
			...
			sequence.Definition = backup.SequenceDefinition{
				LastVal: 1,
				Increment: 1,
			...
			backup.PrintCreateSequenceStatements(backupfile, tocfile,
				[]backup.Sequence{sequence}, sequenceMetadataMap)
			...
			structmatcher.ExpectStructsToMatchIncluding(&sequenceRel,
				&resultSequences[0].Relation, "Name", "Schema")
		...
		It("creates a sequence with privileges, owner, and comment", func() {
			...
			sequenceMetadata := testutils.DefaultMetadata("SEQUENCE",
				true, true, true, includeSecurityLabels)
			...
			uniqueID := testutils.UniqueIDFromObjectName(connectionPool,
				"public", "my_sequence", backup.TYPE_RELATION)
