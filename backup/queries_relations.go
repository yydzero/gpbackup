...
func GetAllSequences(connectionPool *dbconn.DBConn) []Sequence {
	...
	for i := range results {
		if results[i].OwningTable != "" {
			results[i].OwningTable = fmt.Sprintf("%s.%s", results[i].Schema, results[i].OwningTable)
		}
		if results[i].OwningColumn != "" {
			results[i].OwningColumn = fmt.Sprintf("%s.%s", results[i].OwningTable, results[i].OwningColumn)
		}
