...
ALTER TABLE public.testtable REPLICA IDENTITY USING INDEX testindex;`)
	...
	Context("PrintCreateRuleStatements", func() {
		rule := backup.RuleDefinition{
			Oid: 1,
			Name: "testrule",
			OwningSchema: "public",
			OwningTable: "testtable",
			Def: "CREATE RULE update_notify AS ON UPDATE TO testtable DO NOTIFY testtable;",
		}
		It("can print a basic rule", func() {
			...
			testutils.ExpectEntry(tocfile.PostdataEntries, 0, "public",
				"public.testtable", "testrule", "RULE")
			testutils.AssertBufferContents(tocfile.PostdataEntries, buffer,
	...
	Context("PrintCreateTriggerStatements", func() {
		trigger := backup.TriggerDefinition{
			Oid: 1,
			Name: "testtrigger",
			OwningSchema: "public",
			OwningTable: "testtable",
			Def: "CREATE TRIGGER sync_testtable AFTER INSERT OR DELETE OR UPDATE ON testtable FOR EACH STATEMENT EXECUTE PROCEDURE flatfile_update_trigger()",
		}
		It("can print a basic trigger", func() {
			...
			testutils.ExpectEntry(tocfile.PostdataEntries, 0, "public",
				"public.testtable", "testtrigger", "TRIGGER")
			testutils.AssertBufferContents(tocfile.PostdataEntries, buffer,
		...
		It("can print a trigger with a comment", func() {
			triggers := []backup.TriggerDefinition{trigger}
			triggerMetadataMap := testutils.DefaultMetadataMap("TRIGGER",
				false, false, true, false)
	...
	Context("PrintCreateEventTriggerStatements", func() {
		It("can print a basic event trigger", func() {
			eventTrigger := backup.EventTrigger{
				Oid: 1,
			...
			testutils.ExpectEntry(tocfile.PostdataEntries, 0, "",
				"", "testeventtrigger", "EVENT TRIGGER")
			testutils.AssertBufferContents(tocfile.PostdataEntries, buffer,
