...
var _ = Describe("backup/predata_types tests", func() {
	...
	Describe("PrintCreateEnumTypeStatements", func() {
		enumOne := backup.EnumType{
			Oid: 1,
			Schema: "public",
			Name: "enum_type",
			EnumLabels: "'bar',\n\t'baz',\n\t'foo'",
		}
		enumTwo := backup.EnumType{
			Oid: 1,
			Schema: "public",
			Name: "enum_type",
			EnumLabels: "'bar',\n\t'baz',\n\t'foo'",
	...
	Describe("PrintCreateBaseTypeStatement", func() {
		...
		baseFull := backup.BaseType{
			...
			Preferred: true,
			Delimiter: ",",
			StorageOptions: "compresstype=zlib, compresslevel=1, blocksize=32768",
			Collatable: true,
		}
		basePermOne := backup.BaseType{
			Oid: 1,
			Schema: "public",
			Name: "base_type",
			Input: "input_fn",
...
ALTER TYPE public.base_type
	...
	Describe("PrintCreateTypeStatements", func() {
		shellOne := backup.ShellType{
			Oid: 1,
			Schema: "public",
			Name: "shell_type1",
		}
		baseOne := backup.BaseType{
			Oid: 1,
			Schema: "public",
			Name: "base_type1",
			Input: "input_fn",
		...
		rangeOne := backup.RangeType{
			Oid: 1,
			Schema: "public",
			Name: "range_type1",
		}
		It("prints shell type for a shell type", func() {
			backup.PrintCreateTypeStatements(backupfile, tocfile,
				[]backup.ShellType{shellOne}, []backup.BaseType{}, []backup.RangeType{})
			testutils.ExpectEntry(tocfile.PredataEntries, 0, "public",
				"", "shell_type1", "TYPE")
			testutils.AssertBufferContents(tocfile.PredataEntries, buffer,
				"CREATE TYPE public.shell_type1;")
		})
		It("prints shell type for a base type", func() {
			backup.PrintCreateTypeStatements(backupfile, tocfile,
				[]backup.ShellType{}, []backup.BaseType{baseOne, baseTwo}, []backup.RangeType{})
			testutils.ExpectEntry(tocfile.PredataEntries, 0, "public",
				"", "base_type1", "TYPE")
			testutils.ExpectEntry(tocfile.PredataEntries, 1, "public",
				"", "base_type2", "TYPE")
			testutils.AssertBufferContents(tocfile.PredataEntries, buffer,
				"CREATE TYPE public.base_type1;", "CREATE TYPE public.base_type2;")
		})
		It("prints shell type for a range type", func() {
			backup.PrintCreateTypeStatements(backupfile, tocfile,
				[]backup.ShellType{}, []backup.BaseType{}, []backup.RangeType{rangeOne})
			testutils.ExpectEntry(tocfile.PredataEntries, 0, "public",
				"", "range_type1", "TYPE")
			testutils.AssertBufferContents(tocfile.PredataEntries, buffer,
				"CREATE TYPE public.range_type1;")
		})
	})
	Describe("PrintCreateDomainStatement", func() {
		...
		checkConstraint := []backup.Constraint{{
			Name: "domain1_check",
			ConDef: "CHECK (VALUE > 2)",
			OwningObject: "public.domain1",
		}}
		domainOne := backup.Domain{
			Oid: 1,
			Schema: "public",
			Name: "domain1",
			DefaultVal: "4",
			BaseType: "numeric",
			NotNull: true,
			Collation: "public.mycollation",
		}
		domainTwo := backup.Domain{
			Oid: 1,
			Schema: "public",
			Name: "domain2",
			DefaultVal: "",
		...
		It("prints a domain with a constraint", func() {
			...
			testutils.ExpectEntry(tocfile.PredataEntries, 0, "public",
				"", "domain1", "DOMAIN")
			testutils.AssertBufferContents(tocfile.PredataEntries, buffer,
				`CREATE DOMAIN public.domain1 AS numeric DEFAULT 4 COLLATE public.mycollation NOT NULL
	CONSTRAINT domain1_check CHECK (VALUE > 2);`)
		...
		It("prints a domain without constraint with comment, security label, and owner", func() {
			domainMetadata := testutils.DefaultMetadata("DOMAIN",
	...
	Describe("PrintCreateRangeTypeStatement", func() {
		...
		It("prints a basic range type", func() {
			...
			testutils.ExpectEntry(tocfile.PredataEntries, 0, "public",
				"", "rangetype", "TYPE")
			testutils.AssertBufferContents(tocfile.PredataEntries, buffer,
				`CREATE TYPE public.rangetype AS RANGE (
	SUBTYPE = test_subtype_schema.test_subtype
);`)
		})
	...
	Describe("PrintCreateCollationStatement", func() {
		It("prints a create collation statement", func() {
			collation := backup.Collation{
				Oid: 1, Name: "collation1",
				Collate: "collate1",
				Ctype: "ctype1",
				Schema: "schema1",
			}
			backup.PrintCreateCollationStatements(backupfile, tocfile,
				[]backup.Collation{collation}, emptyMetadataMap)
			testutils.AssertBufferContents(tocfile.PredataEntries, buffer,
		...
		It("prints a create collation statement with owner and comment", func() {
			...
			collationMetadataMap := testutils.DefaultMetadataMap("COLLATION",
				false, true, true, false)
			backup.PrintCreateCollationStatements(backupfile, tocfile,
				[]backup.Collation{collation}, collationMetadataMap)
